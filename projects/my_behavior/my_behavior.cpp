...
void MyBehavior::OnButtonPressed(uint16_t button_index, bool long_press) {
    if (button_index == 0) {
...
void MyBehavior::OnRadioPacketReceived(const IdOnlyState& packet, int8_t rssi) {
    logger->log("Received radio packet with id = %d", packet.id);
    rx_table.AddPacket(packet);
}

void MyBehavior::OnUartCommand(UartCommand& command) {
